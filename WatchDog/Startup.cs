...
namespace WatchDog
{
    ...
    public class Startup
    {
        ...
        public static bool SetStartup(bool runOnStartup)
        {
            if (runOnStartup)
            {
                return IsInStartup()?true:RunOnStartup();
            }
            else
            {
                return IsInStartup()?RemoveFromStartup():true;
            }
        }
        ...
        public static bool IsInStartup(string AppTitle, string AppPath)
        {
            ...
            {
                rk = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", true);
                var AppTitleValue = rk.GetValue(AppTitle);

                if (AppTitleValue == null) return false;
                value = AppTitleValue.ToString();
                if(value == null)
                {
                    return false;
                }
